...
namespace WallopSceneEditor.ViewModels
{
    public class SceneEditViewModel : ViewModelBase
    {
        ...
        public bool FadoutOverlay
        {
            get => _fadoutOverlay;
            set => this.RaiseAndSetIfChanged(ref _fadoutOverlay, value);
        }
        ...
        private async Task Init()
        {
            ...
            try
            {
                ...
                {
                    var engineAppName = appSettings.EngineConfig.InstanceName;
                    ...
                    if (_setup.BoundEngineProcId.Value > 0)
                    {
                        ...
                        engineAppName = await _engineService.GetEngineNameAsync(_setup.BoundEngineProcId.Value, appSettings.ApplicationName).ConfigureAwait(false);
                    }
                    else
                    {
                    ...
                    if (!await _engineService.ConnectAsync(appSettings.ApplicationName, engineAppName).ConfigureAwait(false))
                    {
                ...
                else if (_setup.SceneSource == SceneSources.BoundEngine)
                {
                    ...
                    var reply = await _engineService.SendMessageExpectReplyAsync(new GetSceneMessage(), 15000).ConfigureAwait(false);
